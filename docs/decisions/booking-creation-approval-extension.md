...
## Known gaps, deliberately not addressed this phase

- **No "booking must be in the future" validation.** Nothing stops a member from creating (and paying for) a booking whose window is already in the past. Compounding this, `BookingCancellationService::cancel()` already refuses to cancel a booking once its cancellation window has passed, so a past-dated booking has no way back out once created. Closing this needs a product decision (how far into the past is still tolerable, if at all) that this branch shouldn't make unilaterally.
- **The three new `spaces` columns have no admin API surface.** `requires_approval`, `slot_granularity_minutes`, and `buffer_minutes` can only be set via tinker/SQL — `StoreSpaceRequest`, `UpdateSpaceRequest`, and `SpaceResource` don't reference them. This mirrors the identical, already-accepted gap for `cancellation_window_minutes` from the prior phase, so it's consistent drift rather than new drift.
- **`BookingExtensionService::extend()` only re-checks conflicting-booking overlap**, not business hours or `buffer_minutes`. An extension can therefore push `end_at` past the branch's closing time, and the buffer gap re-check finding from Task 9 stands as accepted. The phase brief's lock-and-recheck table deliberately scopes extension's re-validation more narrowly than creation's, so this is scoped-as-designed rather than an oversight.
