...
**Tables:** `users` · spatie role tables · `user_branch_memberships` · `otp_verifications` · `audit_logs` (spatie/activitylog, already installed and migrated) · `notifications` · `private_office_requests` · `companies` · `company_user` (`door_access_enabled`) · `guests` · `consents`.

**Locked decisions:** OTP is the verified identity channel (§5.1) · company accounts are created by operations only, never self-service · #9 a guest holds no account and acts only through a host · every sensitive action is audit-logged.

**Placeholders:** `private_office_requests.converted_company_id` nullable — the request precedes the company by design.

**Acceptance:** a real member logs in by OTP; operations creates a company from a signed request and adds members; every code issuance, door open, permission change and maintenance assignment appears in `audit_logs`.

**Guards:** no route creates a staff, admin or company account without an authenticated admin · every action on the sensitive list writes an audit row (assert the list, not one example) · `guests` cannot be the actor on any request.
