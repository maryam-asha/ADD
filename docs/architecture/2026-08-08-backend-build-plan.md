...
app/
  Domain/
    Foundation/     branches, buildings, floors, zones, spaces, resources, seats_desks
...
### Phase 1 — Foundation: the spatial hierarchy · **DONE**

**Tables:** `branches` · `buildings` · `floors` (new) · `zones` (new) · `spaces` (+`zone_id`, `event_hall`, `allocation_model`, `pricing_currency`, `status_reason`, `status_from`, `status_until`) · `resources` (new, +operational status per D.11) · `seats_desks` (new; `qr_point_id` nullable until Phase 7).

**Locked decisions:** #1 `Branch` keeps its name and is the top level · #2 Floor/Zone are classification only · #3 Resource is metadata, never booked · #4 Seat/Desk is an address, not a seat map · #8 operational status at Space level, no hierarchical escalation · #13 `is_lockable` true only for room / business / event_hall.

**Documented deviation:** PRD decision #1 names an *eight*-level hierarchy with `District` above `Branch`. A `districts` table and `branches.district_id` shipped with this phase, then were deliberately removed — see [district-removed.md](../decisions/district-removed.md). The hierarchy is seven levels; `Branch` is the top. This is a conscious departure from decision #1's literal count, not an oversight.
...
**Placeholders:** `spaces.allocation_model` is nullable — the space_type → allocation_model mapping is Phase 5 business logic, not this structural phase.
...
**Acceptance:** a real member logs in by OTP and gets the `member` role, not `operations` (regression-tested after the rename) · operations creates a company from a signed (quoted) request and the source request closes to `contracted` in the same transaction · a request that isn't yet quoted is rejected · a company member with `door_access_enabled` is allowed by `CompanyPolicy`, one without the flag or without membership is denied, `admin` bypassed via the `Gate::before` that existed at the time (since removed — see [rbac-permission-pilot.md](../decisions/rbac-permission-pilot.md); `admin` now needs a real `is_admin=true` pivot row like anyone else) · a member creates a guest and a `guest_data_on_behalf` consent is recorded automatically · a member cannot see or delete another member's guest · every one of: role change, status change, company creation, door-access toggle, member added/removed writes an `activity_log` row with the actor and a before/after payload.
...
### Phase 3 — Membership, plans, wallet · **DONE**
...
**Guards:** unused included hours do not survive a cycle boundary · no money column is float · a `is_subscription=false` plan can never create a membership row · every `wallets`/`memberships` row has a non-null `owner_type`+`owner_id`, and `owner_type='company'` requires an `active` company. Per this codebase's `tests/Guards/` convention (static source scans only), the last two are behavioral, not static, so they're enforced as a `static::saving()` model hook (`App\Domain\Membership\Concerns\ValidatesOwner`, plus a second hook on `Membership` for the `is_subscription` check) and proven in `tests/Feature/Membership/{WalletOwnershipGuardTest,MembershipSubscriptionGuardTest}.php`, not in `tests/Guards/`.

**New design work this phase had to resolve that neither existing decision doc covered:** the base schema for `plans`/`memberships`/`wallets` beyond the `owner_type`/`owner_id` shape, the no-stored-balance approach (every balance is a live bcmath sum over `wallet_transactions`, never a cached column), and — the part with no written precedent at all — the debit-resolution algorithm for when more than one categorized/restricted grant of the same category coexists in one wallet with different `wallet_transaction_allowed_users` restrictions. All three are written up in [phase-3-membership-plan-wallet-mechanics.md](../decisions/phase-3-membership-plan-wallet-mechanics.md), including the one instruction this phase received that stayed genuinely ambiguous after being raised and not answered — whether `CompanyController::store` should also grant a starter membership, not just a wallet — resolved there in the wallet-only direction, flagged rather than silently assumed.

**Bugs found and fixed while building this phase:** `WalletService::attemptCategoryDebit()`'s first version under-counted an eligible category balance whenever two or more credit grants shared the same restriction signature (e.g. two unrestricted grants in the same category) — it summed historical debits once per signature but then applied that *whole* total to *each* same-signature credit independently, rather than treating them as one pool, so the apparent remaining balance was short by (n-1) × the debited amount. Caught in code review with a direct repro before anything downstream depended on it (nothing in this phase's own endpoints calls the multi-grant path yet — only `debitGeneral`/`creditCategorized` are wired to real routes today), fixed by grouping credits by signature before applying each signature's debit total once, and locked in with a regression test (`WalletServiceTest::test_two_unrestricted_credits_sharing_a_signature_are_one_pool_not_double_counted`). Also fixed: a duplicate id in `StoreWalletAllocationRequest`'s `user_ids` reached the `wallet_transaction_allowed_users` unique index unfiltered and surfaced as a raw 500 instead of a 422 (added a `distinct` rule); `MemberAuthController::verifyOtp`'s new wallet-provisioning line wasn't transaction-wrapped, so a failure between user creation and wallet creation could have left a user with no wallet — wrapped both in one `DB::transaction()`.

**Known follow-up, not fixed in this phase (flagged, not silently decided):** `CompanyPolicy::manageMembers` and the membership-purchase/wallet-allocation endpoints don't re-check `Company::status` at spend time — only `Wallet` creation checks it once. A company deactivated after it already has a wallet can still have its admin purchase memberships and allocate wallet credit indefinitely. None of the three decision docs address whether a deactivated company should lose spending capability immediately or only stop future contract activity; needs a decision before Phase 5 wires real spending through this.
